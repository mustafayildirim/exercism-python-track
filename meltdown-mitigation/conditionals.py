...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    if temperature < 800 and neutrons_emitted > 500:
        product_of_temperature_neutrons_emitted = temperature * neutrons_emitted
        if product_of_temperature_neutrons_emitted < 500000:
            return True
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    """
    percentage_of_value = ((voltage * current)/theoretical_max_power)*100
    if percentage_of_value >= 80:
        return 'green'

    if 60 <= percentage_of_value < 80:
        return 'orange'

    if 30 <= percentage_of_value <60:
        return 'red'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if calculation < threshold * 0.9:
        return 'LOW'

    if threshold * 0.9 < calculation < threshold * 1.1:
        return 'NORMAL'
